...
def factor_product(x, y):
    ...
    xy, xy_in_x_ind, xy_in_y_ind = np.intersect1d(x.get_variables(), y.get_variables(),
                                                  return_indices=True)  # np.intersectld获得交集，排序后的
    ...
    x_not_in_y = np.setdiff1d(x.get_variables(), y.get_variables(),
                              assume_unique=True)  # 把在a中的但是不在b中的元素按a中的顺序排序，并且不合并重复的元素
    y_not_in_x = np.setdiff1d(y.get_variables(), x.get_variables(), assume_unique=True)

    x_mask = np.isin(x.get_variables(), xy,
                     invert=True)  # 当参数invert被设置为True时，如果a中元素在b中没有出现则返回True,如果出现了则返回False，结果形状与a一致
...
def read_2d(m, n, path):
    fi = open(path, "r")
    data = []
    for line in fi.readlines():
        line = line.strip()
        list_line = line.split(' ')
        list_line = [float(_) for _ in list_line]
...
def read_3d(x, y, z, path):
    ...
    for line in fi.readlines():
        line = line.strip()
        list_line = line.split(' ')
        list_line = [float(j) for j in list_line]
        data.append(list_line)

    data = np.array(data)
    DATA = np.zeros((x, y, z))
    for i in range(len(DATA)):
        DATA[i, :, :] = data[i * x:(i + 1) * y, :]

    # return DATA
    return np.array(DATA, dtype=np.float16)


def read_5d(x, y, z, m, n, path):
    fi = open(path, "r")
    data = []
    for line in fi.readlines():
        line = line.strip()
    ...
    for l in range(x):
        ...
        for k in range(y):
            ...
            for j in range(z):
                data_m = []
                for i in range(m):
                    data_m.append(data[i + j * m + k * z * m + l * y * z * m])
                data_z.append(data_m)
            data_y.append(data_z)
        data_x.append(data_y)

    # return np.array(data_x)
    return np.array(data_x, dtype=np.float16)
